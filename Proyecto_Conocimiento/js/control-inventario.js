...
function calcularValorParte(parte) {
    ...
    var costoUnitario = parte.precio_unitario;
    var cantidad = parte.cantidad;

    var valorTotal = costoUnitario * cantidad;
...
function registrarEntrada(idParte, cantidad) {
    ...
    var stockProyectado = parte.cantidad + cantidad;

    if (stockProyectado > parte.capacidad_maxima) {
        return false;
    }

    parte.cantidad = stockProyectado;
...
function verificarStockMinimo(parte) {
    ...
    if (parte.cantidad <= parte.alerta_stock) {
    return true;
} else {
    return false;
}
